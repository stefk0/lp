...
%%
%% дърво_мин(T,A) - елементът A е най-малкият елемент в подреденото двоично дърво T
%%
%% Условие - Известна е структурата на дървото T
%% 
...
%%
%% дърво_елем(T,E) - елементът E е връх в подреденото двоично дърво T
%%
...
%%
%% дърво_подредено1(T) - T е дърво с подредба да елементите ляв-корен-десен
%% 
%% Условие - известни са елемените на T
%% 
дърво_подредено1(nil).
дърво_подредено1(tree(_,nil,nil)).
дърво_подредено1(tree(X,L,nil)) :- дърво_подредено1(L), дърво_макс(L,Y), Y =< X.
дърво_подредено1(tree(X,nil,R)) :- дърво_подредено1(R), дърво_мин(R,Z), X =< Z.
дърво_подредено1(tree(X,L,R)) :- дърво_подредено1(L), дърво_подредено1(R), дърво_макс(L,Y), дърво_мин(R,Z), Y =< X, X =< Z.


дърво_подредено(nil).
дърво_подредено(tree(_,nil,nil)).
дърво_подредено(tree(X,L,nil)) :- Y #=< X, дърво_подредено(L), дърво_макс(L,Y).
дърво_подредено(tree(X,nil,R)) :- X #=< Z, дърво_подредено(R), дърво_мин(R,Z).
дърво_подредено(tree(X,L,R)) :- Y #=< X, X #=< Z, дърво_подредено(L), дърво_подредено(R), дърво_макс(L,Y), дърво_мин(R,Z).
...
%%
%% Балансирано дърво - двоично дърво, в което разликата във височините на лявото и дясното поддърво е не повече от единица
%% 
...
%%
%% списък_дърво_баланс(L,T) - T е балансирано подредено двоично дърво съдържащ точно елементите на списъка L
%%
%% Условие - известно е дървото T или списъка L
%% 
списък_дърво_баланс(L,T) :- равен_брой(T,L), дърво_балансирано(T), дърво_списък(T,S), дърво_подредено(T), разбърк(L,S).
%%
%% Важна ли е подредбата на конюнктите в дефиницията на списък_дърво_баланс ?
%% 
...
%% Ротация наляво
%% 
ребаланс(tree(A,T1,T2), tree(B,tree(A,T,L),R)) :- N #= M,  N + 1 #= K,
                                                  ребаланс(T1,T), ребаланс(T2,tree(B,L,R)),
...
%% Ротация надясно
%% 
ребаланс(tree(A,T1,T2), tree(B,L,tree(A,R,T))) :- N #= K,  N + 1 #= M,
                                                  ребаланс(T1,tree(B,L,R)), ребаланс(T2,T),
                                                  дърво_височина(T,N), дърво_височина(L,M), дърво_височина(R,K).

%%
%% Двойна ротация наляво
%% 
ребаланс(tree(A,T1,T2), tree(C, tree(A,T,L1), tree(B,R1,R))) :- N + 1 #= M,
                                                                ребаланс(T1,T), ребаланс(T2, tree(B,tree(C,L1,R1),R)),
                                                                дърво_височина(T,N), дърво_височина(tree(C,L1,R1),M).

%%
%% Двойна ротация надясно
%% 
ребаланс(tree(A,T1,T2), tree(C, tree(B,L,L1), tree(A,R1,T))) :- N + 1 #= M,
                                                                ребаланс(T1,tree(B,L,tree(C,L1,R1))), ребаланс(T2,T),
                                                                дърво_височина(T,N), дърво_височина(tree(C,L1,R1),M).


%%
%% Вмъкване на елемент в балансирано дърво
%% 
...
%% списък_дърво_баланс3(L,T) :- равен_брой(T1,L), разбърк(L,S), дърво_списък(T1,S), дърво_подредено(T1), ребаланс(T1,T).



%% списък_дърво_баланс1(L, T) - T е балансирано двоично дърво съставено от елемените на L
%%
%% Условие - известен е списъкът L
%% 
списък_дърво_баланс1(L,T) :- дърво_много_вмък_баланс(L, nil, T).


%%
%% Пример:
%% 
%% ?- списък_дърво_баланс1([4,3,1,2,6,7,9,5],T).
...
%% false.
%%
%% ?- списък_дърво_баланс1(L,tree(2,tree(1,nil,nil),tree(3,nil,nil))).
%% L = [1, 2, 3] ;
%% L = [2, 1, 3] ;
%% L = [3, 1, 2] ;
%% L = [1, 3, 2] ;
...
%%
%% списък_дърво_баланс2(L, T) - T е балансирано двоично дърво съставено от елемените на L
%%
%% Условие - известно е дървото T или списъкът L
%% 
списък_дърво_баланс2(L, T) :- равен_брой(T,L), дърво_много_вмък_баланс(L,nil,T).
%%
%% Защо този предикат работи много по-неефективно от списък_дърво_баланс1 ?
%%
%% ?- списък_дърво_баланс2(L,tree(2,tree(1,nil,nil),tree(3,nil,nil))).
%% L = [1, 2, 3] ;
%% L = [2, 1, 3] ;
%% L = [3, 1, 2] ;
%% L = [1, 3, 2] ;
...
списък_дърво_баланс3(L, T) :- равен_брой(T,L), дърво_балансирано(T), дърво_много_вмък_баланс(L,nil,T).
%%
%% Защо списък_дърво_баланс3 е по-ефективен от списък_дърво_баланс2 ?
%% 
